...
class VersionManager {
    ...
    async createVersion(request) {
        return (0, database_js_1.pgTransaction)(async (client) => {
            ...
            const docResult = await client.query(`SELECT * FROM documents WHERE id = $1 AND deleted_at IS NULL`, [request.documentId]);
            ...
            await client.query(`UPDATE documents SET is_latest_version = FALSE, updated_at = NOW() WHERE id = $1`, [request.documentId]);
            // Insert new version as latest
            const newDocId = (0, uuid_1.v4)();
            ...
            await blockchainService.recordCustodyEvent({
                txType: database_js_2.CustodyAction.VERSION_CREATE,
            ...
            const newDocResult = await client.query(`SELECT * FROM documents WHERE id = $1`, [newDocId]);
    ...
    async getVersionHistory(documentId) {
        const result = await (0, database_js_1.pgQuery)(`SELECT * FROM document_versions WHERE document_id = $1 ORDER BY version ASC`, [documentId]);
        return result.rows;
    }
    async getVersion(documentId, version) {
        const result = await (0, database_js_1.pgQuery)(`SELECT * FROM document_versions WHERE document_id = $1 AND version = $2`, [documentId, version]);
        return result.rows[0] || null;
    }
    async getLatestVersion(documentId) {
        const result = await (0, database_js_1.pgQuery)(`SELECT * FROM documents WHERE id = $1 AND is_latest_version = TRUE AND deleted_at IS NULL`, [documentId]);
        return result.rows[0] || null;
    }
    // ========================================================================
    // GET ALL VERSIONS OF A DOCUMENT LINEAGE
    // ========================================================================
    async getDocumentLineage(documentId) {
        // Find root document
        let rootResult = await (0, database_js_1.pgQuery)(`SELECT * FROM documents WHERE id = $1`, [documentId]);
        if (rootResult.rows.length === 0)
            return [];
        let rootDoc = rootResult.rows[0];
        while (rootDoc.parent_document_id) {
            const parentResult = await (0, database_js_1.pgQuery)(`SELECT * FROM documents WHERE id = $1`, [rootDoc.parent_document_id]);
            if (parentResult.rows.length === 0)
                break;
            rootDoc = parentResult.rows[0];
        }
        // Get all descendants
        const result = await (0, database_js_1.pgQuery)(`SELECT * FROM documents WHERE parent_document_id = $1 OR id = $1 ORDER BY version ASC`, [rootDoc.id]);
        return result.rows;
    }
    // ========================================================================
    // REVERT TO PREVIOUS VERSION
    // ========================================================================
    async revertToVersion(documentId, targetVersion, userId, reason) {
        return (0, database_js_1.pgTransaction)(async (client) => {
            // Get target version
            ...
            await client.query(`UPDATE documents SET is_latest_version = FALSE, updated_at = NOW() WHERE id = $1`, [currentDoc.id]);
    ...
    toVersion(doc) {
        return {
            id: (0, uuid_1.v4)(), // Would be the actual version record ID
            document_id: doc.id,
            version: doc.version,
            file_hash_sha256: doc.file_hash_sha256,
