...
exports.EntityGraphService = void 0;
exports.getEntityGraphService = getEntityGraphService;
...
class EntityGraphService {
    ...
    // ========================================================================
    async fetchDocumentEntities(caseId, include) {
        if (!include)
            return [];
        const result = await (0, database_js_1.pgQuery)(`SELECT id, title, document_number, extracted_entities
             FROM documents
             WHERE case_id = $1 AND deleted_at IS NULL AND is_latest_version = TRUE
             AND extracted_entities IS NOT NULL
             AND extracted_entities != '{}'`, [caseId]);
    ...
    // ========================================================================
    async fetchEvidenceEntities(caseId, include) {
        if (!include)
            return [];
        const result = await (0, database_js_1.pgQuery)(`SELECT e.id, e.name, e.evidence_number, em.extracted_entities
             FROM evidence e
             LEFT JOIN evidence_metadata em ON em.evidence_id = e.id
             WHERE e.case_id = $1 AND e.deleted_at IS NULL
             AND em.extracted_entities IS NOT NULL`, [caseId]);
        return result.rows.map(row => ({
            id: row.id,
            name: row.name,
    ...
    // PROCESS DOCUMENT ENTITIES
    // ========================================================================
    processDocumentEntities(doc, entityMap, edgeMap, entityTypes) {
    ...
    // PROCESS EVIDENCE ENTITIES
    // ========================================================================
    processEvidenceEntities(evi, entityMap, edgeMap, entityTypes) {
        const allowedTypes = entityTypes || ['PERSON', 'ORGANIZATION', 'LOCATION', 'DATE', 'LEGAL_REFERENCE'];
    ...
    // ========================================================================
    createCoOccurrenceEdges(resource, allEntities, currentEntityKey, currentValue, entityMap, edgeMap, currentNodeType) {
        const typeMap = {
            persons: 'PERSON',
            organizations: 'ORGANIZATION',
            locations: 'LOCATION',
    ...
    // GRAPH ALGORITHMS
    // ========================================================================
    async findShortestPath(caseId, sourceEntity, targetEntity) {
        ...
        const queue = [{ node: sourceEntity, path: [sourceEntity] }];
        const visited = new Set([sourceEntity]);
    ...
    async getCentralEntities(caseId, topN = 10) {
        const graph = await this.buildGraph({ caseId, includeDocuments: true, includeEvidence: true });
        // Degree centrality
...
exports.EntityGraphService = EntityGraphService;
// ============================================================================
// SINGLETON INSTANCE
// ============================================================================
let entityGraphServiceInstance = null;
function getEntityGraphService() {
    if (!entityGraphServiceInstance) {
        entityGraphServiceInstance = new EntityGraphService();
    }
    return entityGraphServiceInstance;
}
//# sourceMappingURL=entity-graph.service.js.map